...
import {RecentlyPlayedComponent} from './routes/recently-played/recently-played.component';
...
@Component({
  selector: 'app-root',
  imports: [RouterOutlet, SelectPlaylistComponent, RecentlyPlayedComponent],
  template: `
    <div
      ...
      @if (playlistService.setupDone) {
        <div class="flex gap-2">
          ...
          <!--@else {
            <h1 class="bg-blue-500 cursor-pointer text-white p-2 rounded-md no-drag" (click)="openRecentlyPlayedList()">Recently
              Played</h1>
      }!-->
          ...
          <h1 class="bg-yellow-500 cursor-pointer text-white p-2 rounded-md no-drag" (click)="resetRound()">Next
            song</h1>
    ...
    @if (showRecentlyPlayed) {
      <app-recently-played (closed)="closeRecentlyPlayed()"/>
    } @else {
